...
class MainActivity : AppCompatActivity() {
    ...
    private fun restoreBottomNavState() {
        when (currentBottomNavTab) {
            BottomNavTab.FIREWALL -> {
    ...
    private fun showFirewallTab() {
        ...
        // Asegurar que el fragment container está completamente oculto
    ...
    private fun showSettingsTab() {
        // Ocultar firewall container primero
        val firewalContainer = findViewById<View>(R.id.firewall_content_container)
        firewalContainer?.visibility = View.GONE
    ...
    override fun onBackPressed() {
        // Si hay fragmentos en el back stack, popear primero
        if (supportFragmentManager.backStackEntryCount > 0) {
            supportFragmentManager.popBackStack()
            // Mostrar el tab de firewall después de popear
            showFirewallTab()
            bottomNavigation.selectedItemId = R.id.nav_firewall
        } else {
            super.onBackPressed()
